#> iris_bob:get_targeted_block
#
# Returns the position of the block targeted
# To tell which block a player is facing, anchoring to the eye position is needed: execute as <player> at @s anchored eyes positioned ^ ^ ^ run function iris_bob:get_target_block
#
# @api
...
function iris_bob:get_coordinates/main
...
execute as @e[type=minecraft:marker, tag=iris.ray] at @s run function iris_bob:raycast/loop